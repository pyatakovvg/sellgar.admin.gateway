...
export class BrandService {
  ...
  async update(uuid: string, dto: UpdateBrandDto, files: ProductUploadFileDto[] = []) {
    if (uuid !== dto.uuid) {
      throw Error('Not persistent');
    }

    const existingBrand = await this.findByUuid(uuid);
    const uploadedFiles: FileRecord[] = [];

    try {
      const preparedDto = await this.prepareImage(dto, files, uploadedFiles);
      const message = this.productService.send({ cmd: 'brand.update' }, preparedDto);
      ...
      await validateOrReject(resultInstance);
      await this.deleteRemovedImage(existingBrand, preparedDto);
  ...
  async create(dto: CreateBrandDto, files: ProductUploadFileDto[] = []) {
    const uploadedFiles: FileRecord[] = [];

    try {
      const preparedDto = await this.prepareImage(dto, files, uploadedFiles);
      const message = this.productService.send({ cmd: 'brand.create' }, preparedDto);
  ...
  private async prepareImage<T extends CreateBrandDto | UpdateBrandDto>(
    dto: T,
  ...
  private async deleteRemovedImage(existingBrand: BrandEntity, dto: UpdateBrandDto) {
    const before = existingBrand.image?.imageUuid;
